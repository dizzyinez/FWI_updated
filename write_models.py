...
def get_nproc(folder, param="x"):
    return len(glob.glob(os.path.join(folder, "proc??????_{}.bin".format(param))))
...
def read_all_from_folder(folder, param, nproc=None):
    ...
    if nproc == 0:
        raise Exception("No '{}' data found in folder: {}".format(param, folder))
...
def get_typed_args(args, types):
    ...
    for arg, t in zip(args, types):
        try:
            ...
            raise Exception("{} is not in correct type: {}".format(arg, t))
    return typed_args
...
if __name__ == "__main__":
    ...
    for model_type in model_types.keys():
        ...
        for model_param in model_params:
            parser.add_argument(
                "--{model_type}-{model_param}".format(
                    model_type=model_type, model_param=model_param
                ),
                metavar=metavar,
                nargs=len(metavar),
                type=arg_type,
                help="{model_type} {model_param} Model".format(
                    model_type=model_type.title(), model_param=model_param.title()
                ),
            )
    ...
    for model_type, model_func in model_types.items():
        metavar, types = zip(*model_args[model_type])
        for model_param in model_params:
            arg = getattr(args, "{}_{}".format(model_type, model_param))
            if arg is not None:
